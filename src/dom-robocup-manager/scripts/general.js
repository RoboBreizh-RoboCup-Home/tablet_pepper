...
current_task_listener.subscribe(function (message) {
    if (String(message.data).slice(-6) == ".exec;") {
        ...
        var children = document.getElementById('task-items').children;
        var children_list = [];
        for (var i = 0; i < children.length; i++) {
            children_list.push(children[i].innerHTML);
        }
        if ((replacement_text[title][String(message.data)] != last_task) && (children_list.indexOf(replacement_text[title][String(message.data)]) == -1)) {
            if ((replacement_text[title][String(message.data)] == "") || (replacement_text[title][String(message.data)] == undefined)) {
                return;
            }
